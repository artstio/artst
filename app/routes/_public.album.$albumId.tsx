...
import { AlbumHeroHeader } from '~/components/spotify/album/hero';
...
export const loader = async ({ context: { visitorId }, params }: LoaderFunctionArgs) => {
					...
					<div className="space-y-4 md:space-y-8 lg:space-y-12">
						<div>
							...
							<div className="space-y-1">
								<Typography variant="h2" aria-label="Album name" className="leading-tight">
									{album.name}
								</Typography>
								<Typography variant="h2" aria-label="Album release date" className="leading-tight">
									{getReleaseDate(album.releaseDate)}
								</Typography>
								<Typography variant="h2" aria-label="Album duration" className="leading-tight">
									{formatDuration(album.tracks.reduce((total, curr) => total + curr.durationMs, 0))}
								</Typography>
							</div>
